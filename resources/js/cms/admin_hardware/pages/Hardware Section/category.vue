                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            <tr v-for="singleData in allData.data" :key="singleData.id">
                                ...
                                <td v-if="singleData.label">{{ singleData.label }}</td>
                                <td v-else><span class="error--text"> Not Found </span></td>
                                

                                <td>
                                    <span v-if="singleData.dependences.length">
                                        <span v-for="(item, index) in singleData.dependences" :key="index">
                                            <span class="p-1 m-1 rounded-pill small">{{ item.name }}, </span> 
                                        </span>
                                    </span>
                                    <span v-else>
                                        <span class="text-danger">No dependences</span>
                                    </span>
                                </td>
                                ...
                                <td class="text-center">
                                    ...
                                    <span v-else><span class="error--text"> Not Found </span></span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <v-form v-model="valid" ref="form">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            ...
                            <v-row>
                                ...
                                <v-col cols="12">
                                    <v-text-field v-model="form.name" label="Enter Category Name" :rules="categoryRules" required></v-text-field>
                                </v-col>
                                <v-col cols="12">
                                    <v-text-field v-model="form.label" label="Enter Category Label" ></v-text-field>
                                </v-col>
