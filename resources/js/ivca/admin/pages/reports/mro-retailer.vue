                            ...
                            <tr v-for="singleData in allData.data" :key="singleData.id">
                                ...
                                <td>
                                    <span v-for="(item, i) in singleData.audits_retailer" :key="i">
                                        <!-- {{ item.id }}, -->
                                        <button v-if="item.status == 1" @click="singleAuditShow(item.id)" class="btn btn-info btn-sm m-1">Audit by {{ item.auditordata.name }}</button>
                                    </span>
                                </td>

                                <td>

                                    <!-- {{ singleData.audits_retailer.length }} -->
                                    <button v-if="singleData.audits_retailer.length > 0"
...
<script>
    ...
    import singleAuditReport from './single-report/retailer.vue';
    ...
    export default {
        ...
        data() {
            return {
                ...
                // current page url
                currentUrl: '/ivca/admin/reports/retailer',
            ...
            getDirectResults(page = 1) {
                ...
                axios.get(this.currentUrl + '/index?page=' + page +
                    ...
                    .then(response => {
                        ...
                        // if(response.data.audits_retailer){

                        //     if(response.data.audits_retailer[0].status == 1){
