        ...
        <div v-if="allData.data">
            ...
            <v-card v-for="singleData in allData.data" :key="singleData.id" class="mb-3">
                <v-card-title class="justify-center">
                    <v-row>
                        <v-col cols="8">
                            {{ singleData.car.name }} || {{ singleData.car.number }} <v-badge v-if="singleData.status == 1" color="teal" content="Booked" bordered ></v-badge> <v-badge v-else color="error" content="Cancelled" bordered></v-badge>
                        </v-col>
                        <v-col cols="4" class="text-right">
                            <v-btn color="teal white--text" small depressed @click=" getBookbyModalData( singleData.bookby.id)">
                ...
                <v-card-text>
                    <v-row>
                        <v-col>
                            <div>
                                Driver: <v-btn color="indigo white--text" small depressed @click=" getDriverModalData( singleData.driver.id)">
                        ...
                        <v-col class="text-right">
                            <div>
                                Gasoline: <span v-if="singleData.gasoline">{{ singleData.gasoline }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                Octane: <span v-if="singleData.octane">{{ singleData.octane }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                Toll: <span v-if="singleData.toll">{{ singleData.toll }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                Mileage: <span v-if="singleData.start_mileage"> {{ singleData.start_mileage }} || {{ singleData.end_mileage }} </span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                KM: <span v-if="singleData.km">{{ singleData.km }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                Total: <span v-if="singleData.cost">{{ singleData.cost }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                            <div>
                                Rating: <span v-if="singleData.driver_rating">{{ singleData.driver_rating }}</span> <span v-else class="error--text">Not Found !!</span>
                            </div>
                        </v-col>
                        
                    </v-row>
                    <div>
                                Booking Duration: <span class="orange--text">{{ singleData.start  | moment("MMMM Do YYYY, h:mm a") }} </span> to <span class="orange--text"> {{ singleData.end  | moment("MMMM Do YYYY, h:mm a") }} </span>
                            </div>
                ...
                <v-card-text>
                    <v-list-item three-line>
                        <v-list-item-content class="text-overline">
                            ...
                            <div>
                                License: <span v-if="driverData.license">{{ driverData.license }}</span> <span v-else class="error--text">Not Available !</span>
                            </div>
            ...
            getCarData() {
                axios.get(this.currentUrl + '/car-data').then(response => {
                    ...
                    for (let i = 0; i < response.data.length; i++) {
                        ...
                        this.carData[i] = {value: response.data[i].id, text: response.data[i].name + ' || ' + response.data[i].number };
