...
namespace App\Http\Controllers\Sms\Admin\User;
...
class IndexController extends Controller
{
    ...
    public function index(){
        ...
        // Query
        $allDataQuery = User::with('sms_roles')
            ->where('delete_temp', '!=', '1')
            ->whereHas( 'roles', function($query){
                $query->whereIn( 'name', ['Sms', 'Sms-admin'] ); // role with no admin
            });
    ...
    public function roles_data(){
        $allData = SmsOperation::orderBy('name')->get();
        return response()->json($allData, 200);
    }
    ...
    public function roles_update(Request $request){
        ...
        if($id){
            ...
            $success = $user->sms_roles()->sync($request->roles);
