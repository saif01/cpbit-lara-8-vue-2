...
class TokenController extends Controller
{
    ...
    public function create(Request $request){
       ...
       $checkUserRole = User::whereHas('ivca_roles', function($query){
           $query->whereIn('name', ['Admin', 'Auditor', 'Mro']);
       })->count();

       if($checkUserRole > 0){

        $data = new ivcaAuditMroToken();
    ...
    public function check(Request $request){
        ...
        $date = date('Y-m-d');
        $data = ivcaAuditMroToken::where('id', $id)->where('token', $token)->where('date', $date)->first();
    ...
    public function check_by_user(Request $request){
        ...
        $date = date('Y-m-d');
        $data = ivcaAuditMroToken::where('token', $token)
        ->where('date', $date)
    ...
    public function data(Request $request){
        ...
        $date = date('Y-m-d');
        $data = ivcaAuditMroToken::where('token', $token)->where('date', $date)->first();
