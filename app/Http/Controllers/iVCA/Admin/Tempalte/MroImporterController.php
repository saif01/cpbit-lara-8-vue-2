...
class MroImporterController extends Controller
{
    ...
    public function store(Request $request){
       ...
       // $tblCheck = ivcaTemplateMroImporter::exists();

        $dataCount = ivcaTemplateMroImporter::count();

        if($dataCount > 0){
            $data = ivcaTemplateMroImporter::orderBy('id', 'desc')->first();
        }else{
            $data = new ivcaTemplateMroImporter();
        }
