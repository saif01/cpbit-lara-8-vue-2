...
class MroRetailerController extends Controller
{
    ...
    public function store(Request $request){
       ...
       // $tblCheck = ivcaTemplateMroRetailer::exists();

        $dataCount = ivcaTemplateMroRetailer::count();

        if($dataCount > 0){
            $data = ivcaTemplateMroRetailer::orderBy('id', 'desc')->first();
        }else{
            $data = new ivcaTemplateMroRetailer();
        }
